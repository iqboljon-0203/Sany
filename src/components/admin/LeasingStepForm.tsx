...
export default function LeasingStepForm({ initialData }: { initialData?: any }) {
  ...
  const [formData, setFormData] = useState({
    step_num: initialData?.step_num || 1,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <h3 className="text-xl font-bold text-gray-900 mb-6">Lizing bosqichi ma'lumotlari</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Bosqich raqami</label>
            <input required type="number" name="step_num" value={formData.step_num} onChange={handleChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-sany-red outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Tartib raqami (order)</label>
